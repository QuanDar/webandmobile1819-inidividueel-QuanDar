...
export function fetchMessages() {
    return function (dispatch) {
        axios.get('http://localhost:8000/api/messagesWithComments')
            .then((response) => {
...
export function fetchMessageByContent(query) {
    return function (dispatch) {
        axios.get('http://localhost:8000/api/messages/content/'+query)
            .then((response) => {
...
export function fetchMessageByContentAndCategory(content, category) {
    return function (dispatch) {
        axios.get('http://localhost:8000/api/messages/content-and-category/'+content+'/'+category)
            .then((response) => {
...
export function deleteMessage(id) {
    return function (dispatch) {
        axios.delete('http://localhost:8000/api/messages/delete/'+id)
            .then((response) => {
                dispatch({type: types.DELETE_MESSAGE, payload: response.data, id: id})
            })
            .catch((err) => {
                throw(err);
            })
...
export function upvoteMessage(id) {
    return function (dispatch) {
        axios.put('http://localhost:8000/api/messages/upvote/'+id)
            .then((response) => {
...
export function downvoteMessage(id) {
    return function (dispatch) {
        axios.put('http://localhost:8000/api/messages/downvote/'+id)
            .then((response) => {
...
export function postComment(messageId, comment) {
    ...
    return function (dispatch) {
        axios.post('http://localhost:8000/api/messages/comment/post/', bodyFormData)
            .then((response) => {
